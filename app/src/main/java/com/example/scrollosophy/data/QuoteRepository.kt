...
class QuoteRepository(context: Context) {
    ...
    suspend fun fetchQuote(): Quote {
        return suspendCancellableCoroutine { continuation ->
            ...
            val callback = getCallbackObject(continuation)
    ...
    // ChatGPT
    private fun getCallbackObject(continuation: CancellableContinuation<Quote>): UrlRequest.Callback {
        val callback = object : UrlRequest.Callback() {
            ...
            override fun onSucceeded(request: UrlRequest?, info: UrlResponseInfo?) {
                try {
                    val jsonObject = org.json.JSONObject(responseString.toString()).getJSONObject("quote")
                    val quote =
                        Quote(
